...
url = "https://raw.githubusercontent.com/IMXEren/rvx-builds/customs/.env"
def get_patches_json_from_sources():
    r = requests.get(url)
    data = r.text
    pattern = r'(\S+_PATCHES_JSON_DL)=(\S+)'
    matches = re.findall(pattern, data, re.MULTILINE)
    patch_json_dl_values = {match[0].lower(): match[1] for match in matches}
    for key, value in patch_json_dl_values.items():
        value = value.strip()
        if value.startswith("https://github"):
            version = latest(repo=value, assets_filter="json$", output_format="assets")
            print(version[0])
            print(f"{key}: {value}")
        else:
            print(f"{key} does not contain 'github.com'")
get_patches_json_from_sources()
exit()
import re
import requests
...
rv_json_url = "https://github.com/YT-Advanced/ReX-patches/releases/download/v2.187.2/patches.json"
rvx_json_url = "https://github.com/YT-Advanced/ReX-patches/releases/download/v2.187.2/patches.json"
